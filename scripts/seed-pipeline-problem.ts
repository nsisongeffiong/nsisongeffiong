/**
 * Seed script — inserts The Pipeline Problem ideas post
 *
 * Usage:
 *   npx tsx --env-file=.env scripts/seed-pipeline-problem.ts
 */
...
const post = {
  ...
  content: `<p>Everyone is debating the wrong question.</p>

<p>"Will AI replace software engineers?" gets the panels, the LinkedIn posts, the think pieces from people who last touched a codebase in 2009. It's the kind of question that sounds urgent but doesn't actually require an answer to do damage. While the debate runs, something quieter and more specific is already in motion: AI is eating the bottom of the pipeline, and the top is staying intact.</p>
